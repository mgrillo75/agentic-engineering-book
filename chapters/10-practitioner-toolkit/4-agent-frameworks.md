...
Before selecting an orchestration framework, practitioners face a more fundamental choice: which type of execution environment to build within. The three-tier landscape (full agentic harnesses, web-based chat interfaces, constrained agentic environments) determines the capability ceiling before any framework decision is made. Choosing a chat interface as the deployment environment is not a model choice — it is a capability ceiling.
...
- **To [Orchestrator Pattern](../7-patterns/3-orchestrator-pattern.md)**: Frameworks implement orchestrator patterns differently—LangGraph via graphs, CrewAI via crews, AutoGen via conversations
