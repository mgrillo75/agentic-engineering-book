...
Side effects on the higher layers, such as appending to the coordination log, merging memory, or updating an issue, must never roll back core state. If the coordination service is unreachable, the run still succeeds. If a memory merge fails, the change still lands. These side effects _enhance_ the system; they do not _gate_ it.
